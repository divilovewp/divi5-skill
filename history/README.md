...
**Not archived:** 0.4.0 (Divi 5.6.2) and 0.5.0 (Divi 5.7.0) — no standalone snapshots were kept; they
exist only as entries in `../CHANGELOG.md`. The lineage then continued to the current **0.5.x** line
(Divi 5.7.x) in the repo root. (0.5.0 → 0.5.1 was only a `builderVersion` stamp bump, so the current
root closely reflects 0.5.0.)
